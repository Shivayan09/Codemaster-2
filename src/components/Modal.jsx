...
const Modal = ({ show, onClose, title, description, youtubeId, docsUrl}) => {
      ...
      <div className="bg-black/70 p-6 rounded-3xl h-[77vh] w-[70vw] md:w-[50vw] shadow-white shadow-xs">
        ...
        <div className="sources p-3">
          <ul className='flex justify-evenly flex-wrap gap-5'>
            {['description', 'youtube', 'documentation'].map(tab => (
        ...
        <div className="p-4 h-[80%] overflow-y-auto text-[0.95rem] text-gray-400">
          ...
          {activeTab === 'documentation' && (
            <div>
              <a href={docsUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">
                Open {title} documentation on MDN
              </a>
            </div>
          )}
        </div>
